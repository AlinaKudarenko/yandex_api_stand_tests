...
# Функция для изменения значения в параметре firstName
...
# Функция для негативной проверки (ошибка связана с введенными символами)
def negative_assert_symbol(first_name):
    # В переменную user_body сохраняется изменённое тело для запроса на создание пользователя
    user_body = get_user_body(first_name)
    ...
    # Проверяет, что в теле ответа атрибут message содержит текст
    assert response.json()["message"] == "Имя пользователя введено некорректно. " \
                                         "Имя может содержать только русские или латинские буквы," \
                                         " длина должна быть не менее 2 и не более 15 символов"


# Функция для негативной проверки (не все необходимые парметры были переданы)
...
# Тест 12. Ошибка
# Параметр fisrtName имеет другой тип
